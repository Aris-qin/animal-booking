// ==================== GitHub配置 ====================
// ⚠️ 修改这三行！
const GITHUB_USER = 'Aris-qin';  // 改为你的GitHub用户名
const GITHUB_REPO = 'animal-booking';      // 改为你的仓库名
const GITHUB_TOKEN = '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';           // 改为你复制的token
...
let lastSync = 0;
const SYNC_INTERVAL = 5000; // 5秒自动同步一次
...
async function loadData() {
    try {
        ...
        console.log('✗ 无法连接GitHub，使用本地缓存:', e.message);
...
async function saveData() {
    ...
    // 防止频繁上传GitHub（每次间隔至少3秒）
    if (Date.now() - lastSync < 3000) {
        return;
    }
    
    try {
        // 第1步：获取当前文件的SHA（需要用来更新）
        ...
        const updateResponse = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                message: `Update cage data: ${new Date().toLocaleString('zh-CN')}`,
                content: btoa(JSON.stringify(cageData, null, 2)),  // Base64编码
                sha: sha
            })
        });
        ...
        console.log('⚠ 无法上传到GitHub，数据已保存本地:', e.message);
    }
}
